...
( function( $ ) {
	...
	function init( _options ) {
		...
		root = that
			.empty()
			.html(
				'<div class="' + rootClass + '" id="' + options.id + '">' +
					'<ul class="regions">' +
					'</ul>' +
				...
				if( !inputBlocked() ) {
					...
					switch( e.keyCode ) {
						case 8: // Backspace
							...
							break;
						case 13: 	// return
							var cmd = cursor.parent().find( '.content' ).text();
							commandTranslate( cmd, root.parent() );
							root.trigger( 'console_put', [ { line: e.keyCode, done: cmd.length > 0 } ] );
							return false;
					}
				}
			} );
	...
	function overlayReposition( reset ) {
		...
		var
			yOffset = 0,
			xOffset = root.find( '.cursor' ).offset().left - 10;
		
		if( reset !== true ) {
			var
				cursor = $( '.cursor' );
			yOffset = cursor.offset().top - 39;
		}
